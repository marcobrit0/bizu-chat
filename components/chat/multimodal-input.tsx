...
function PureMultimodalInput({
    ...
    (cmd: SlashCommand) => {
      ...
      switch (cmd.action) {
        ...
        case "clear":
          ...
          break;
        case "rename":
          toast(ui.input.renameFromSidebar);
          break;
        case "model": {
        ...
        case "theme":
          ...
          break;
        case "delete":
          toast(ui.input.deleteThisChat, {
            action: {
              label: ui.sidebar.deleteConfirm,
              onClick: () => {
                fetch(
                ...
                router.push("/");
                toast.success(ui.sidebar.chatDeleted);
              },
            },
          });
          break;
        case "purge":
          toast(ui.input.deleteAllChats, {
            action: {
              label: ui.input.deleteAll,
              onClick: () => {
                fetch(
                ...
                router.push("/");
                toast.success(ui.sidebar.allChatsDeleted);
              },
            },
          });
          break;
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      toast.error(ui.input.uploadFailed);
    }
  }, []);
  ...
  const handleFileChange = useCallback(
    async (event: ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        toast.error(ui.input.uploadFilesFailed);
      } finally {
        setUploadQueue([]);
      }
    },
  ...
  const handlePaste = useCallback(
    async (event: ClipboardEvent) => {
      ...
      setUploadQueue((prev) => [...prev, ui.input.pastedImage]);
      ...
      try {
        ...
        toast.error(ui.input.pasteUploadFailed);
      } finally {
        setUploadQueue([]);
      }
    },
  ...
  const handlePromptSubmit = useCallback(() => {
    ...
    if (status === "ready" || status === "error") {
      ...
      toast.error(ui.input.waitForModel);
    }
    ...
    <div className={cn("relative flex w-full flex-col gap-4", className)}>
      ...
      >
        ...
        <PromptInputTextarea
          ...
          placeholder={
            editingMessage ? ui.input.editMessage : ui.input.askAnything
          }
          ref={textareaRef}
          value={input}
        />
      ...
      <div className="ml-auto flex items-center gap-2 text-foreground/70">
        {capabilities?.[model.id]?.tools
          ? maybeWithTooltip(
              ...
              ui.input.supportsTools
            )
          : null}
        {capabilities?.[model.id]?.vision
          ? maybeWithTooltip(
              <EyeIcon className="size-3.5" />,
              ui.input.supportsVision
            )
          : null}
        {capabilities?.[model.id]?.reasoning
          ? maybeWithTooltip(
              <BrainIcon className="size-3.5" />,
              ui.input.supportsReasoning
            )
          : null}
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side="right" sideOffset={8}>
        {ui.actions.modelUnavailableDemo}
      </TooltipContent>
    </Tooltip>
  );
}
  ...
  return (
    <ModelSelector onOpenChange={setOpen} open={open}>
      ...
      <ModelSelectorContent commandDefaultValue={selectedModel.id}>
        <ModelSelectorInput placeholder={ui.input.searchModels} />
        <ModelSelectorList>
          {(() => {
