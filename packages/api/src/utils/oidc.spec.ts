...
describe('OpenID Token Utilities', () => {
  ...
  describe('processOpenIDPlaceholders', () => {
    it('should replace BIZU_OPENID_TOKEN with access token', () => {
      const tokenInfo = {
        accessToken: 'access-token-value',
        idToken: 'id-token-value',
        userId: 'oidc-sub-456',
      ...
      const input = 'Authorization: Bearer {{BIZU_OPENID_TOKEN}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);

      expect(result).toBe('Authorization: Bearer access-token-value');
    });

    it('should replace BIZU_OPENID_ACCESS_TOKEN with access token', () => {
      const tokenInfo = {
        accessToken: 'access-token-value',
        userId: 'oidc-sub-456',
      };

      const input = 'Token: {{BIZU_OPENID_ACCESS_TOKEN}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);

      expect(result).toBe('Token: access-token-value');
    });

    it('should replace BIZU_OPENID_ID_TOKEN with id token', () => {
      const tokenInfo = {
        idToken: 'id-token-value',
        userId: 'oidc-sub-456',
      };

      const input = 'ID Token: {{BIZU_OPENID_ID_TOKEN}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);

      expect(result).toBe('ID Token: id-token-value');
    });

    it('should replace BIZU_OPENID_USER_ID with user id', () => {
      const tokenInfo = {
        userId: 'oidc-sub-456',
      };

      const input = 'User: {{BIZU_OPENID_USER_ID}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);

      expect(result).toBe('User: oidc-sub-456');
    });

    it('should replace BIZU_OPENID_USER_EMAIL with user email', () => {
      const tokenInfo = {
        userEmail: 'test@example.com',
        userId: 'oidc-sub-456',
      };

      const input = 'Email: {{BIZU_OPENID_USER_EMAIL}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);

      expect(result).toBe('Email: test@example.com');
    });

    it('should replace BIZU_OPENID_USER_NAME with user name', () => {
      const tokenInfo = {
        userName: 'Test User',
        userId: 'oidc-sub-456',
      };

      const input = 'Name: {{BIZU_OPENID_USER_NAME}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);
    ...
    it('should replace multiple placeholders in a single string', () => {
      ...
      const input =
        'Authorization: Bearer {{BIZU_OPENID_TOKEN}}, ID: {{BIZU_OPENID_ID_TOKEN}}, User: {{BIZU_OPENID_USER_ID}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);
    ...
    it('should replace empty string when token field is undefined', () => {
      ...
      const input =
        'Access: {{BIZU_OPENID_TOKEN}}, ID: {{BIZU_OPENID_ID_TOKEN}}, User: {{BIZU_OPENID_USER_ID}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);
    ...
    it('should handle all placeholder types in one value', () => {
      ...
      const input = `
        Authorization: Bearer {{BIZU_OPENID_TOKEN}}
        ID Token: {{BIZU_OPENID_ID_TOKEN}}
        Access Token (alt): {{BIZU_OPENID_ACCESS_TOKEN}}
        User ID: {{BIZU_OPENID_USER_ID}}
        User Email: {{BIZU_OPENID_USER_EMAIL}}
        User Name: {{BIZU_OPENID_USER_NAME}}
        Expires: {{BIZU_OPENID_EXPIRES_AT}}
      `;
    ...
    it('should handle case-sensitive placeholders', () => {
      ...
      const input = 'Token: {{bizu_openid_token}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);

      expect(result).toBe('Token: {{bizu_openid_token}}');
    });
    ...
    it('should handle multiple occurrences of the same placeholder', () => {
      ...
      const input =
        'Primary: {{BIZU_OPENID_TOKEN}}, Secondary: {{BIZU_OPENID_TOKEN}}, Backup: {{BIZU_OPENID_TOKEN}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);
    ...
    it('should return original value when tokenInfo is null', () => {
      ...
      expect(result).toBe('Authorization: Bearer {{BIZU_OPENID_TOKEN}}');
    });
