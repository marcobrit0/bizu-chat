import { extractEnvVariable } from 'bizu-data-provider';
...
function processUserPlaceholders(value: string, user?: IUser, isHeader: boolean = false): string {
  ...
  for (const field of ALLOWED_USER_FIELDS) {
    const placeholder = `{{BIZU_USER_${field.toUpperCase()}}}`;
 ...
 * Recognized placeholders: `{{BIZU_BODY_<FIELD>}}` where `<FIELD>` ∈ ALLOWED_BODY_FIELDS.
...
function processBodyPlaceholders(value: string, body: RequestBody): string {
  ...
  for (const field of ALLOWED_BODY_FIELDS) {
    const placeholder = `{{BIZU_BODY_${field.toUpperCase()}}}`;
